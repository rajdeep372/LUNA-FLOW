...
const AnalysisView: React.FC<Props> = ({ logs, profile }) => {
  ...
  const performAnalysis = async () => {
    ...
    try {
      ...
      try {
        const geminiResult = await analyzeHealthRisks(logs, profile.age, profile.location);
        geminiWellnessPlan = geminiResult.wellnessPlan; 
      } catch (geminiError) {
      ...
      setAnalysis({
        ...
        risks: [], 
        wellnessPlan: geminiWellnessPlan, 
      ...
      if (reportElement) {
          setTimeout(async () => {
              try {
                  ...
                  pdf.save('LunaFlow_Health_Report.pdf');
                  
                  setSaveMessage({ type: 'success', text: 'PDF Downloaded Successfully!' });
              } catch (pdfError) {
                  console.error("PDF Generation failed:", pdfError);
                  setSaveMessage({ type: 'error', text: 'Saved, but PDF download failed.' });
              }
          }, 500);
      } else {
