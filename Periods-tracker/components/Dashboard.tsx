...
import { PeriodLog, UserProfile } from '../types';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, Cell } from 'recharts';
import { Calendar, Activity, AlertCircle, TrendingUp, Droplet, ArrowRight, FileText, Clock, X, ShieldCheck, Utensils, Flower2, Apple, CheckCircle2 } from 'lucide-react';
            ...
            {savedReports.map((report, idx) => (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${report.mlPrediction?.prediction === 'None' || report.mlPrediction?.prediction === 'Normal' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                    {report.mlPrediction?.prediction === 'None' ? 'No Risk' : report.mlPrediction?.prediction}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-5">
                  <div className="w-14 h-14 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xl shrink-0 border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                    {report.geminiAnalysis?.overallHealthScore}%
                  </div>
                ...
                <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-rose-500 rounded-3xl p-8 text-white shadow-md flex flex-col justify-center">
                  <div className="flex items-center gap-6 w-full mb-4">
                      <div className="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center shrink-0 border-4 border-white">
                         <span className="text-2xl font-bold">{selectedReport.geminiAnalysis?.overallHealthScore}%</span>
                      </div>
                      <div>
                          <h2 className="text-xl font-bold mb-1 uppercase tracking-tight">Cycle Vitality Score</h2>
                      </div>
                  </div>
                  <p className="text-indigo-50 leading-relaxed text-sm opacity-90">{selectedReport.geminiAnalysis?.summary}</p>
                </div>

                <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-4">
                        <div className={`p-3 rounded-xl ${selectedReport.mlPrediction?.prediction === 'None' || selectedReport.mlPrediction?.prediction === 'Normal' ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
                            <Activity size={24} />
                        </div>
                        <div>
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">ML Pattern Assessment</h3>
                            <p className={`text-2xl font-black ${selectedReport.mlPrediction?.prediction === 'None' || selectedReport.mlPrediction?.prediction === 'Normal' ? 'text-emerald-500' : 'text-rose-500'}`}>
                                {selectedReport.mlPrediction?.prediction === 'None' ? 'No Risk Detected' : selectedReport.mlPrediction?.prediction}
                            </p>
                        </div>
                    </div>
                    <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl text-slate-600 text-sm flex gap-3 items-start mt-auto">
                        ...
                        <p className="leading-relaxed text-xs">{selectedReport.mlPrediction?.warning}</p>
                    </div>
                </div>
              </div>

              {/* Risks Section */}
              {selectedReport.geminiAnalysis?.risks?.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedReport.geminiAnalysis.risks.map((risk: any, idx: number) => (
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                    ...
                    <div className="space-y-4">
                      {selectedReport.geminiAnalysis?.wellnessPlan?.dietChart?.map((item: any, idx: number) => (
                  ...
                  <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedReport.geminiAnalysis?.wellnessPlan?.yogaPoses?.map((pose: any, idx: number) => (
                ...
                <div className="space-y-6">
                  <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm h-full">
                    ...
                    <ul className="space-y-4">
                      {selectedReport.geminiAnalysis?.wellnessPlan?.foodHabits?.map((habit: string, idx: number) => (
